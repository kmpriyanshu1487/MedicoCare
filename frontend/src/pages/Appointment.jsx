...
const Appointment = () => {
  ...
  const getAvailableSlot = async () => {
    setDocSlots([]); // Reset slots before loading
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0">
          <p className="flex items-center gap-2 text-2xl font-medium text-gray-900">
            {docInfo.name}
            <img className="w-5" src={assets.verified_icon} alt="Verified Info" />
          </p>
      ...
      <div className='sm:ml-72 sm:pl-4 font-medium text-gray-700'>
        ...
        <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
          {
            docSlots.length > 0 && docSlots.map((item, index) => (
              <div
                ...
                className={`text-center py-5 min-w-20 rounded-full cursor-pointer ${slotIndex === index ? 'bg-primary text-white' : 'border border-gray-200'}`}
                key={index}
              >
        ...
        <button className='btn text-gray-600 text-sg font-light px-14 py-3 rounded-full my-6'>Book an Appointment</button>
      </div>
