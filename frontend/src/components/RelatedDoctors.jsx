    ...
    useEffect(() => {
        if (doctors.length > 0 && speciality) {
            const doctorsData = doctors.filter((doc) =>
                doc.speciality === speciality && doc._id !== docId // Corrected filtering logic
            );
    ...
    return (
        <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
            ...
            <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
                {
                    relDoc.slice(0, 5).map((items, index) => (
                        ...
                        >
                            <img src={items.image} alt={`${items.name}'s profile`} className='bg-blue-50' />
                            <div className='p-4'>
            ...
            <button
                ...
                className='btn text-gray-500 px-9 py-2 rounded-full mt-10'
            >
                More
            </button>
        </div>
